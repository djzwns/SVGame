...
{
    /// <summary>
    /// FreeNet 엔진과 유니티 어플리케이션을 이어주는 클래스이다.
    /// FreeNet 엔진에서 받은 접속 이벤트, 메시지 수신 이벤트 등을 어플리케이션으로 전달하는 역할을 하는데
    /// MonoBehaviour 를 상속받아 유니티 어플리케이션과 동일한 스레드에서 작동되도록 구현하였다.
    /// </summary>
    public class CFreeNetUnityService : MonoBehaviour
    {
        ...
        public void connect(string host, int port)
        {
            ...
            {
                Debug.LogError("Alread connected.");
                return;
            }
        ...
        /// FreeNet 엔진의 메시지 송수신 처리는 워커 스레드에서 수행되지만 유니티의 로직 처리는 메인 스레드에서 수행되므로
        /// 큐잉 처리를 통하여 메인 스레드에서 모든 로직 처리가 이루어지도록 구성하였다.
        /// </summary>
        void Update()
        {
            // 수신된 메시지에 대한 콜백
